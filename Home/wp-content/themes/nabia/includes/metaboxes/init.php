...
class cmb_Meta_Box {
	...
	public function register_scripts() {
			...
			if ( ! is_admin() ) {
		   	...
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear', 'nabia' ),
					'defaultString' => __( 'Default', 'nabia' ),
					'pick'          => __( 'Select Color', 'nabia' ),
					'current'       => __( 'Current Color', 'nabia' ),
				) );
			}
		} else {
		...
		wp_register_style( 'cmb-styles', CMB_META_BOX_URL . 'metabox-style'. $min .'.css', $styles );
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'nabia' ) );
